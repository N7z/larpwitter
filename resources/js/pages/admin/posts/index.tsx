        ...
        <AdminLayout>
            ...
            <form onSubmit={submitSearch} className="mb-4 flex gap-2">
                <input
                    ...
                    className="w-full max-w-xs rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none"
                />
            ...
            <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
                {posts.data.length === 0 ? (
                    ...
                    posts.data.map((post) => (
                        <div key={post.id} className="flex items-start justify-between gap-4 border-b border-gray-200 p-4 last:border-b-0">
                            <div className="flex min-w-0 items-start gap-3">
                                ...
                                <div className="min-w-0">
                                    <p className="text-xs text-gray-500">
                                        <span className="rounded-full bg-gray-100 px-2 py-0.5 font-medium text-gray-600">{kindLabel(post)}</span>{' '}
                                        <Link
                                    ...
                                    {post.body && <p className="mt-1 line-clamp-2 text-sm text-gray-900">{post.body}</p>}
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                                ...
                                <ConfirmDialog
                                    trigger={
                                        <button type="button" className="rounded-lg px-3 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50">
                                            Delete
                                        </button>
                                    }
